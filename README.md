...
* `session`: This is a [Beaker](http://beaker.groovie.org) Session object which can be used to track information about the connection in question.  We use it internally to direct output to the right place in polling.  Feel free to save your own data - just make sure to call `self.session.save()` if you modify it or your changes will be lost.
...
This returns the data structure which Tornado expects to configure itself, with all requests for '/echoTest' pointed at the EchoHandler class.
For the curious, the structure returned looks like this:
...
* **enabled\_protocols**:  This is a `list` of the Socket.IO protocols the server will respond to requests for.  Clients try them one by one until the server and client both find one they both support.  The possibilities are:
